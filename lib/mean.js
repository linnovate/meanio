    ...
    Meanio.Singleton.config = new Config(function(err, config) {
      ...
      if (config.https && config.https.port) {
        var httpsOptions = {
          ...
          //ca: fs.readFileSync(config.https.ssl.ca) // Dunno if this is necessary or not. Saw it in other examples.
        };

        for (var o in config.https.ssl)
          if (httpsOptions[o] == undefined)
            httpsOptions[o] = config.https.ssl[o];
            
        var httpsServer = https.createServer(httpsOptions, require('./bootstrap')(options, database));
        Meanio.Singleton.register('https', httpsServer);
        Meanio.Singleton.register('httpsOptions', httpsOptions);
        httpsServer.listen(config.https.port);
        if (config.https.forceRedirect)
          app.get('*',function(req,res){  
             res.redirect('https://'+req.hostname+(config.https.port==443?'':':'+config.https.port)+req.url)
          });
      } else {
        Meanio.Singleton.register('https', false);
        Meanio.Singleton.register('httpsOptions', false);
      }
