...
function onInstanceAndConfig(defer, meanioinstance, config, done) {
  ...
  var database = mongoose.connect(defaultConfig.db || '', defaultConfig.dbOptions || {}, function(err, openedDb) {
    if (err) {
      console.error('Error:', err.message);
      return console.error('**Could not connect to MongoDB. Please ensure mongod is running and restart MEAN app.**');
    }

    openedDb.connection.db.openCalled = true;
    ...
    Q.allSettled (db_promises).then(dataBasesReady.bind(null, defer, openedDb));
  });
};
