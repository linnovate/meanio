...
function PackageDir(ext,rootpath,name,options){
  AssetDir.call(this,ext,path.join(rootpath,'public'),name,options);
}
...
function configureApp(Meanio,meanioinstance,defer,app){
  Meanio.modules.dependableConstructor().prototype.aggregatePackage = function(){
    var options = {};
    options.aggregate = Meanio.Singleton.getConfig().aggregate;
    options.debug = Meanio.Singleton.getConfig().debug;
    if(Meanio.Singleton.getConfig().assets) {
      options.hash = Meanio.Singleton.getConfig().assets.hash === false ? false : true;
      options.separator = Meanio.Singleton.getConfig().assets.separator || '?v=';
    } else {
      options.hash = true;
      options.separator = '?v=';
    }
  ...
  for(var i in assets.core.css){
    meanioinstance.aggregate('css',assets.core.css[i],{group:'header',singlefile:true},meanioinstance.getConfig());
  }
  for(var i in assets.core.js){
    meanioinstance.aggregate('js',assets.core.js[i],{group:'footer',singlefile:true,global:true,weight:-1000000+parseInt(i)},meanioinstance.getConfig());
  }
...
function supportAggregate(Meanio) {
  ...
  Meanio.prototype.aggregated = function(ext, group, callback) {
    ...
    if (Meanio.Singleton.getConfig().aggregate === false){
      return callback('');
    }
  ...
  Meanio.prototype.Module.prototype.aggregateAsset = function(type, asset, options) {
    ...
    var assetOptions = {};
    if(Meanio.Singleton.getConfig().assets) {
      assetOptions.hash = Meanio.Singleton.getConfig().assets.hash === false ? false : true;
      assetOptions.separator = Meanio.Singleton.getConfig().assets.separator || '?v=';
    } else {
    ...
    if (Meanio.Singleton.getConfig().public &&
      Meanio.Singleton.getConfig().public.languages &&
      Meanio.Singleton.getConfig().public.currentLanguage) {

      var languages = Meanio.Singleton.getConfig().public.languages;
      var currentLanguage = Meanio.Singleton.getConfig().public.currentLanguage;
