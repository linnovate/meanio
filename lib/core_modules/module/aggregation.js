...
WeightedList.prototype.addToScalars = function(content){
  ...
  if(content.src){
    if(content.options.hash && content.hash) {
      ...
      this.hash = md5(this.hash === null ? content.hash : this.hash + content.hash);
    } else {
...
WeightedList.prototype.getHash = function(){
  if(this.dirty){
    this.buildScalars();
  }
  return this.hash;
...
function supportAggregate(Meanio) {
  ...
  Meanio.prototype.aggregatedsrc = function(ext, group, callback) {
    ...
    if (Meanio.Singleton.config.clean.aggregate !== false){
      if(ext==='js'){
        var hash = aggregated.js[group].getHash();
        var enableHash = true;
        var separator = '?v=';
        var version = '';
        if(Meanio.Singleton.config.clean.assets) {
          enableHash = Meanio.Singleton.config.clean.assets.hash === false ? false : true;
          separator = Meanio.Singleton.config.clean.assets.separator || '?v=';
        }
        if (enableHash && hash !== null) {
          version = separator + hash;
        }
        if(group==='header'){
          return callback(['/modules/aggregated.js' + version + (version == '' ? '?' : '&') + 'group=header']);
        }else{
          return callback(['/modules/aggregated.js' + version]);
        }
      }else if(ext==='css' && group==='header'){
        var hash = aggregated.css.getHash();
        var enableHash = true;
        var separator = '?v=';
        var version = '';
        if(Meanio.Singleton.config.clean.assets) {
        ...
        return callback(['/modules/aggregated.css' + version]);
      }
      return callback([]);
    }
  ...
  Meanio.prototype.Module.prototype.aggregateAsset = function(type, asset, options) {
    ...
    this.settings(function(err, settings) {
      ...
      if (options.inline){
        ...
        var fpath = path.join(self.loadedmodule.path('public'),'assets',type,asset);
        switch(type){
          case 'css':
            new CSSFileAsset(fpath,path.join(self.name,'assets',type,asset),options);
            return;
          case 'js':
            new JSFileAsset(fpath,path.join(self.name,'assets',type,asset),options);
            return;
        }
      }
    });
