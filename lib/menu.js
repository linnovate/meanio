...
function supportMenus(Meanio) {
  ...
  Menus.prototype.get = function(options) {
    ...
    var defaultMenu = options.defaultMenu || [];
    options.defaultMenu = [];

    for (var i = 0; i < defaultMenu.length; i++) {
      if (defaultMenu[i].roles.indexOf('all') > -1 || options.roles.indexOf('all') > -1)
        options.defaultMenu.push(defaultMenu[i]);
      else
        for (var j = 0; j < defaultMenu[i].roles.length; j++) {
          if (options.roles.indexOf(defaultMenu[i].roles[j]) > -1)
            options.defaultMenu.push(defaultMenu[i]);
        }
    }
